...
export default class Cache extends Source {
    ...
    recordIdFromKey(type: string, keyName: string, keyValue: string): string {
        ...
        assert('No `keyMap` has been assigned to the Cache, so `recordIdFromKey` can not work.', !!keyMap);
    ...
    peekRecordData(type: string, id: string): InitializedRecord | undefined {
        deprecate('Cache#peekRecordData is deprecated. Call `getRecordData` instead.');
        return this.#sourceCache.getRecordSync({ type, id });
    }
    ...
    query<RequestData extends RecordQueryResult<Model> = RecordQueryResult<Model>>(
    ...
    findRecords(
        ...
        return this.query(q => q.findRecords(typeOrIdentities), options) as Model[];
    }
    ...
    _lookupQueryResult(result: RecordQueryResult<InitializedRecord>, isArray: boolean): RecordQueryResult<Model> {
        if (isArray) {
            if (Array.isArray(result)) {
                return (result as RecordQueryExpressionResult[]).map(i => this.lookupQueryExpressionResult(i));
            } else {
                throw new Assertion('A resultset for a query with an array of `expressions` should also be an array.');
            }
        } else {
            return this.lookupQueryExpressionResult(result as RecordQueryExpressionResult<InitializedRecord>);
        }
    }
    ...
    _lookupTransformResult(
        ...
        if (isArray) {
            if (Array.isArray(result)) {
                return (result as RecordOperationResult[]).map(i => this.lookupOperationResult(i));
            } else {
                throw new Assertion(
    ...
    private lookupQueryExpressionResult(
        ...
        if (Array.isArray(result)) {
            return result.map(i => (i ? this.lookup(i) : i)) as Model[];
        } else if (result) {
    ...
    private lookupOperationResult(result: RecordOperationResult<InitializedRecord>): RecordOperationResult<Model> {
        if (result) {
            return this.lookup(result);
        } else {
            return result;
